...
public class EpisodeSequenceIdGenerator implements IdentifierGenerator {
	private static final Logger LOGGER = LoggerFactory.getLogger(EpisodeSequenceIdGenerator.class);
	...
	public String getEmployeeId(SharedSessionContractImplementor session, Object object) {
		String employeeId = null;
		Episode episode = (Episode) object;// cast to emp object
		int presentYear = Calendar.getInstance().get(Calendar.YEAR);
		GlobalSequenceCounters employeeCounter = TechBeanUtil.getBean(EmrMasterService.class).getEpisodeCounter(episode.getOrgId(), episode.getLocationId());
